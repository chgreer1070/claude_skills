...
description: Use this agent when the user asks to "validate refactoring", "verify refactoring complete", "check refactoring quality", or wants to ensure a refactoring effort achieved its goals. Trigger after refactoring execution to validate results.\n\n<example>\nContext: User completed refactoring and wants validation\nuser: "I've finished the refactoring tasks, can you verify everything is correct?"\nassistant: "I'll use the refactor-validator agent to perform comprehensive validation of the refactoring results."\n<commentary>\nPost-refactoring validation request triggers the validator agent.\n</commentary>\n</example>\n\n<example>\nContext: User wants to check quality after changes\nuser: "Does the plugin structure look correct after the refactoring?"\nassistant: "I'll use the refactor-validator agent to validate the plugin structure and quality."\n<commentary>\nQuality check request after changes triggers validation.\n</commentary>\n</example>\n\n<example>\nContext: User needs to verify no regressions\nuser: "Make sure the refactoring didn't break anything"\nassistant: "I'll use the refactor-validator agent to check for regressions and validate all components."\n<commentary>\nRegression check request triggers comprehensive validation.\n</commentary>\n</example>
model: sonnet
color: yellow
