...
- `$accent` - Accent color
- `$foreground` - Default text/foreground base color
- `$text` - Main text color (auto contrast)
- `$text-muted` - Muted text
- `$foreground-muted` - Dimmed foreground

Additional variables exist in the full theme system — base semantic colors (`$warning`, `$error`,
`$success`), muted variants (`$primary-muted`, etc.), color shades (`$primary-lighten-1/2/3`,
`$primary-darken-1/2/3`), text-on-color aliases (`$text-primary`, `$text-error`, etc.), and
component-specific variables (`$border`, `$border-blurred`, `$scrollbar`, `$link-color`, etc.).
Run `textual colors` from the command line to preview all theme variables interactively.
SOURCE: <https://textual.textualize.io/guide/design/> (accessed 2026-05-07)
