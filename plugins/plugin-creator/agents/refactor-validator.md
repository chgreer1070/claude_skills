...
description: 'Use this agent when the user asks to "validate refactoring", "verify refactoring complete", "check refactoring quality", or wants to ensure a refactoring effort achieved its goals. Trigger after refactoring execution to validate results. <example> Context: User completed refactoring and wants validation user: "I''ve finished the refactoring tasks, can you verify everything is correct?" assistant: "I''ll use the refactor-validator agent to perform comprehensive validation of the refactoring results." <commentary> Post-refactoring validation request triggers the validator agent. </commentary> </example> <example> Context: User wants to check quality after changes user: "Does the plugin structure look correct after the refactoring?" assistant: "I''ll use the refactor-validator agent to validate the plugin structure and quality." <commentary> Quality check request after changes triggers validation. </commentary> </example> <example> Context: User needs to verify no regressions user: "Make sure the refactoring didn''t break anything" assistant: "I''ll use the refactor-validator agent to check for regressions and validate all components." <commentary> Regression check request triggers comprehensive validation. </commentary> </example>'
model: sonnet
color: yellow
---
