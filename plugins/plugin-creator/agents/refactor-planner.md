...
description: 'Use this agent when the user asks to "plan a refactoring", "analyze plugin for refactoring", "create refactoring plan", "assess plugin quality", or wants to prepare a plugin for systematic refactoring. Trigger when breaking down a large refactoring effort into executable tasks. <example> Context: User wants to refactor a plugin user: "I need to refactor the python3-development plugin - it''s gotten too large" assistant: "I''ll use the refactor-planner agent to analyze the plugin and create a comprehensive refactoring plan." <commentary> User requests plugin refactoring analysis, trigger refactor-planner to create structured plan. </commentary> </example> <example> Context: User needs to split oversized skills user: "The main skill in my plugin is over 800 lines, how should I split it?" assistant: "I''ll use the refactor-planner agent to analyze the skill domains and propose a split strategy." <commentary> Skill size concern triggers analysis and planning. </commentary> </example> <example> Context: User wants quality assessment before refactoring user: "Before I start refactoring, can you assess what needs to change?" assistant: "I''ll use the refactor-planner agent to generate a comprehensive assessment and task plan." <commentary> Pre-refactoring assessment request triggers the planning agent. </commentary> </example>'
model: sonnet
color: cyan
---
