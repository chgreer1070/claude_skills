...
flowchart TD
    ...
    L1L2 --> L1["Layer 1 — Script validation (parallel):<br>uv run scripts/create_plugin.py validate ./plugins/{plugin-name}<br>uv run scripts/plugin_validator.py batch ./plugins/{plugin-name}"]
...
```mermaid
flowchart TD
    Start(["Documentation complete"]) --> Check1{"Check 1 — Works Check:<br>Run: uv run scripts/plugin_validator.py ./plugins/{plugin-name}<br>Exit code = 0?"}
...
flowchart TD
    ...
    Q2 -->|"Schema and structure validation"| Scripts["Run validation scripts directly:<br>plugin_validator.py or create_plugin.py validate"]
...
|---|---|---|
| `plugin_validator.py` | `uv run scripts/plugin_validator.py {path}` | Exit code 0 = pass; non-zero = error codes with file:line |
