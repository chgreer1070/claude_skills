...
if TYPE_CHECKING:
    ...
    from .operations import BacklogListItem as _BacklogListItem, ImpactRadiusItem as _ImpactRadiusItem
...
def _item_field_text(item: _BacklogListItem, field: str) -> str:
    """Return the casefolded text for a single field of an item dict."""
    return str(item.get(field, "") or "").casefold()


def _build_haystack(item: _BacklogListItem) -> str:
...
def _item_matches_term(item: _BacklogListItem, term: str, haystack: str | None = None) -> bool:
...
class _Predicate:
    ...
    def __call__(self, item: _BacklogListItem, haystack: str) -> bool:
...
def _apply_search_filter(items: list[_BacklogListItem], search: str) -> list[_BacklogListItem]:
...
def _collect_regex_matches(
    item: _BacklogListItem, term: str, pattern_str: str, snippet_context: int = _DEFAULT_SNIPPET_CONTEXT
...
def _collect_plain_matches(
    item: _BacklogListItem, term: str, snippet_context: int = _DEFAULT_SNIPPET_CONTEXT
) -> list[dict[str, str]]:
...
def _enrich_with_match_context(
    items: list[_BacklogListItem], search: str | None, snippet_context: int = _DEFAULT_SNIPPET_CONTEXT
...
def _dedup_by_issue_number(items: list[_BacklogListItem]) -> list[_BacklogListItem]:
...
async def backlog_list(
    ...
    all_items: list[_BacklogListItem] = result.get("items", [])
    ...
    enriched_items: list[dict[str, object]] | list[_BacklogListItem]
    ...
    if item_depth > 0:
        enriched_items = [_apply_item_depth({**it}, item_depth) for it in enriched_items]
