...
class ListItemsResult(TypedDict, total=False):
    ...
    Used by server.py to remove cast() call sites (T05).
    ``messages``, ``warnings``, and ``errors`` come from ``Output.to_dict()``
    via ``**`` spread and are therefore ``NotRequired``.
    """
...
def _build_list_entry(item: BacklogItem, status_map: dict[int, IssueStatus]) -> BacklogListItem:
