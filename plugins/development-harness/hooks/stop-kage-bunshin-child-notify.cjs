...
/**
 * Stop hook — child-session notification writer.
 *
 * Fires when a kage-bunshin child session stops. Appends a JSONL entry to the
 * parent session's notifications file so the master Stop hook can detect that
 * a child has stopped and inspect its pane for interactive states.
 *
 ...
 *   echo '{"hook_event_name":"Stop","stop_hook_active":false,"session_id":"child456"}' \
 *     | node ./hooks/stop-kage-bunshin-child-notify.cjs
 */
...
process.stdin.on('end', () => {
  ...
  // Avoid writing notifications when stop hook is recursively active
