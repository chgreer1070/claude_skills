...
Create GitHub Releases with changelogs for every calendar day that had commits. Commits are automatically categorized by conventional commit type (feat/fix/refactor/docs/etc.).
...
**Output:**
- Git tags named `daily-YYYY-MM-DD` pointing to the last commit of each day
- GitHub Releases with categorized changelogs (grouped by feat/fix/refactor/docs/test/ci/chore)
- Release title: `Daily Release - YYYY-MM-DD`
- All releases created incrementally (re-running is safe, skips existing dates)
...
## Daily Release Workflow (Details)

### How create_daily_releases.py Works

The script automates daily releases end-to-end:

1. **Group commits by UTC date** from git log on specified branch
2. **Parse conventional commit prefixes** (feat/fix/refactor/docs/test/ci/chore)
3. **Categorize commits** into the `format_mr_description.py` schema (bug_fixes, enhancements, tech_debt, documentation, testing, build_ci, non_functional)
4. **Generate markdown changelog** by calling `format_mr_description.py`
5. **Create git tag** named `daily-YYYY-MM-DD` pointing to last commit of that day
6. **Push tag** to remote with `git push origin <tag>`
7. **Create GitHub Release** with the rendered markdown as release notes
...
```bash
$ python scripts/create_daily_releases.py --dry-run --start-date 2026-02-21 --end-date 2026-02-21
Fetching commits by day...
Found 47 days with commits, processing 1 days
DRY RUN - No changes will be made

Day: 2026-02-21
Tag: daily-2026-02-21
Commit: 2ad91d7a9f4c5e1d2b8c9a0f1e2d3c4b5a6f7e8d
Commits: 42
Markdown preview (first 500 chars):
# Daily Release - 2026-02-21

## Summary
Daily changes from 42 commits
...
### Full Backfill Example

```bash
# Create all releases for entire history
$ python scripts/create_daily_releases.py
Fetching commits by day...
Found 47 days with commits, processing 47 days

Processing 2026-02-01... [======>                    ] 5/47
Processing 2026-02-02... [==========>                ] 10/47
Processing 2026-02-03... [================>          ] 15/47
...
Green] All releases created successfully!

# Check what was created
$ gh release list | head -10
daily-2026-02-21        Daily Release - 2026-02-21   main   about 1 minute ago
daily-2026-02-20        Daily Release - 2026-02-20   main   about 2 minutes ago
daily-2026-02-19        Daily Release - 2026-02-19   main   about 3 minutes ago
...

# View a specific release
$ gh release view daily-2026-02-21
```

### Incremental Runs

The script is safe to run multiple times. By default (`--skip-existing` is True):

```bash
# First run: creates releases for days 1-47
$ python scripts/create_daily_releases.py

# Later run: skips days 1-47 because tags exist, only processes new commits
$ python scripts/create_daily_releases.py
Fetching commits by day...
Found 48 days with commits, processing 1 days  (day 48 is new)
```

To force re-creation of existing releases, use `--skip-existing=false`:

```bash
python scripts/create_daily_releases.py --skip-existing=false
```

### CLI Reference: create_daily_releases.py

**Usage:**

```bash
python scripts/create_daily_releases.py [OPTIONS]
```

**Options:**

- `--start-date TEXT` - Only process days on or after this date (YYYY-MM-DD). Default: all history
- `--end-date TEXT` - Only process days on or before this date (YYYY-MM-DD). Default: today
...
```bash
# Backfill all history with dry-run preview
...
# Force re-creation of existing releases
python scripts/create_daily_releases.py \
  --skip-existing=false \
  --branch main
```
...
### create_daily_releases.py

Creates GitHub Releases for every day with commits.

**Usage:** `python scripts/create_daily_releases.py [OPTIONS]` (see above for details)
...
- **scripts/create_daily_releases.py**: Python CLI to create daily GitHub releases
