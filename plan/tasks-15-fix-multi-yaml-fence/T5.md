...
`TASK_FILE_FORMAT.md` at `plugins/development-harness/docs/TASK_FILE_FORMAT.md` is the canonical format
...
Add a new `## Authorized Writers` section to `plugins/development-harness/docs/TASK_FILE_FORMAT.md` after the
...
- `plugins/development-harness/docs/TASK_FILE_FORMAT.md` — read before editing to locate `## Format Specification`
...
1. Read `plugins/development-harness/docs/TASK_FILE_FORMAT.md` to find `## Format Specification` and the section
...
- Modified file: `plugins/development-harness/docs/TASK_FILE_FORMAT.md`
  - New `## Authorized Writers` section inserted between `## Format Specification` and
    `## Markdown Body Sections`

### Acceptance Criteria

1. `plugins/development-harness/docs/TASK_FILE_FORMAT.md` contains a `## Authorized Writers` section.
...
6. `uv run prek run --files plugins/development-harness/docs/TASK_FILE_FORMAT.md` exits 0.
...
```bash
grep -n "^## " plugins/development-harness/docs/TASK_FILE_FORMAT.md | head -20
```
...
```bash
grep -c "implementation_manager.py\|task_status_hook.py\|split_task_file.py\|migrate_task_format.py" plugins/development-harness/docs/TASK_FILE_FORMAT.md
```
...
```bash
uv run prek run --files plugins/development-harness/docs/TASK_FILE_FORMAT.md
```
