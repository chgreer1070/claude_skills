...
### Phase 1 Completion Summary (commit 30066de — frozen)

Phase 1 is complete and must not be re-implemented or modified. The following functions and types exist and are ready to call:
...
class SamTask(BaseModel):
    ...
    # Cross-feature deps: ["#479"]; within-feature: ["T1", "T2"]
```

**`parsing.py` lines 873-968** — three Phase 1 functions:

- `parse_sam_task_metadata(body: str) -> SamTask | None` — extracts YAML from `<!-- sam:task\n{YAML}\n-->` block. Pattern: `_SAM_TASK_RE = re.compile(r"<!--\s*sam:task\s*\n(.*?)\n-->", re.DOTALL)`. Returns `None` when block is absent or YAML is invalid.
- `build_sam_task_body(task, description, acceptance_criteria) -> str` — builds issue body with `## What`, `## Acceptance Criteria` sections and an invisible `<!-- sam:task ... -->` YAML block at the end.
- `build_sam_task_issue_title(task, description) -> str` — returns `f"[{task.feature}/{task.task_id}] {task.task_type}: {description}"`.

**`github.py` lines 25-79** — connection helpers:

- `get_github(repo=DEFAULT_REPO) -> Repository` — raises `GitHubUnavailableError` (a `BacklogError` subclass) when token is absent or connection fails. Use for write operations (`create_sam_task`, `update_sam_task_status`).
- `try_get_github(repo=DEFAULT_REPO) -> Repository | None` — returns `None` on any failure (no token, network error, 10-second timeout). Use for read/query operations (`get_sam_tasks`, `get_ready_sam_tasks`).

---

### File Registry

| File | Status | Purpose | Required by |
|---|---|---|---|
| `.claude/skills/backlog/backlog_core/github.py` lines 455-581 | FROZEN | Phase 1 CRUD functions | T1 (reads), T4 (calls `update_task_status`) |
| `.claude/skills/backlog/backlog_core/models.py` lines 233-263 | FROZEN | `SamTask` Pydantic model | T1, T2 |
| `.claude/skills/backlog/backlog_core/parsing.py` lines 873-968 | FROZEN | `parse_sam_task_metadata`, `build_sam_task_body`, `build_sam_task_issue_title` | T1 |
| `.claude/skills/backlog/backlog_core/models.py` lines 157-183 | FROZEN | `Output` type, `BacklogError`, `GitHubUnavailableError` | T1, T2 |
| `.claude/skills/backlog/backlog_core/operations.py` | TARGET (T1) | Add 4 new functions; does not exist yet for SAM operations | T1 writes, T2 reads |
| `.claude/skills/backlog/backlog_core/server.py` | TARGET (T2) | Add 4 MCP tools wrapping T1's operations.py functions | T2 writes |
| `plugins/python3-development/skills/implementation-manager/scripts/implementation_manager.py` | TARGET (T3) | Add `--github` / `--parent-issue` flags + `fetch_tasks_from_github()` | T3 writes |
| `plugins/python3-development/skills/implementation-manager/scripts/task_status_hook.py` | TARGET (T4) | Add `sync_completion_to_github()` + `get_parent_issue_number()` | T4 writes |
| `plugins/python3-development/skills/implementation-manager/scripts/task_format.py` | FROZEN | Shared YAML utilities — `has_yaml_frontmatter`, `normalize_status`, `parse_yaml_frontmatter`, `update_yaml_field` | T3, T4 (import contract must not change) |
| `.claude/skills/implement-feature/SKILL.md` | TARGET (T5) | Update orchestrator docs for MCP-preferred query path | T5 writes |
| `plugins/python3-development/skills/development/implement-feature/SKILL.md` | TARGET (T5) | Mirror of above — must be updated identically | T5 writes |
| `.claude/skills/start-task/SKILL.md` | TARGET (T5) | Update context file write instructions to include `parent_issue_number` | T5 writes |
| `plugins/python3-development/skills/development/start-task/SKILL.md` | TARGET (T5) | Mirror of above — must be updated identically | T5 writes |
| `plugins/python3-development/scripts/migrate_tasks_to_github.py` | TARGET (T6) | New migration script — does not exist yet | T6 writes |
| `plan/architect-migrate-sam-task-github-subissues.md` | REFERENCE | Architecture spec — interface contracts (section 3), error strategy (section 5), testing strategy (section 7) | All tasks |
| `plan/codebase/sam-task-migration.md` | REFERENCE | Codebase analysis — existing code shapes, patterns, line numbers | All tasks |
| `plan/research/comparison-github-task-dependency-options.md` | REFERENCE | Dependency model research — Option B selected; PyGitHub sub-issue API verified | T1, T3 |

---

### How the Current System Works (Before Phases 2-5)

The SAM workflow maintains two disconnected tracking systems. At the story level, GitHub Issues are managed by the backlog MCP via `.claude/skills/backlog/backlog_core/`. At the task level, local `plan/tasks-{N}-{slug}.md` files (YAML frontmatter format) are managed by `implementation_manager.py` and `task_status_hook.py`. No connection exists between these two layers.

**Task file format.** Each task is a YAML frontmatter block delimited by `---` within the task file. Required fields: `task`, `title`, `status`, `agent`, `priority`, `complexity`, `dependencies`, `skills`. Optional fields written by hooks: `started`, `completed`, `last_activity`. Two organizational structures are supported: a single monolithic file (multiple `---`-delimited blocks) or a directory with one file per task.

**Readiness query flow.** The orchestrator (`implement-feature` skill) invokes `implementation_manager.py ready-tasks . {slug}`. This script uses `sys.path.insert(0, str(Path(__file__).resolve().parent))` to add its `scripts/` sibling to the path, then imports `task_format` module utilities. `find_task_file_by_slug()` (lines 750-771) searches `plan/` directories for matching task files. `parse_task_file()` reads all YAML frontmatter blocks and returns `list[Task]`. `get_ready_tasks(tasks)` (lines 774-809) computes readiness in memory: a task is ready when its status is `NOT_STARTED` and all items in `task.dependencies` map to a terminal status (`COMPLETE`, `DEFERRED`, or `SKIPPED`) in the `status_by_id` dict. Unknown dep IDs resolve to `None`, which is `not in _TERMINAL_STATUSES`, so they block. The JSON output shape is `{"feature": "slug", "ready_tasks": [{"id": "T1", "name": "...", "agent": "...", "skills": [...]}], "count": N}`.

**Task claim flow.** `/start-task` SKILL.md calls `claim-task` CLI command (lines 1065-1093 in `implementation_manager.py`), which atomically writes `status: in-progress` and `started: {timestamp}` to the task YAML. This is the ONLY permitted way to write `in-progress` status. `/start-task` then writes `.claude/context/active-task-{CLAUDE_SESSION_ID}.json` containing `{"task_file_path": "...", "task_id": "T1"}`.

**Task completion flow.** `task_status_hook.py` runs on two events. On `SubagentStop` (lines 396-447): parses the sub-agent's prompt for `/start-task {path} --task {id}` patterns (two regex forms), falls back to reading `.claude/context/active-task-{session_id}.json` if regex fails. Then writes `status: complete` and `completed: {timestamp}` to the task YAML file, and deletes the context file. On `PostToolUse` (lines 450-498): reads the context file for the active task and updates `last_activity: {timestamp}`. Skips silently if task is already `complete`. Both paths use `task_format.py` utilities: `has_yaml_frontmatter()`, `parse_yaml_frontmatter()`, `normalize_status()`, `update_yaml_field()`. Context file path: `cwd / ".claude" / "context" / f"active-task-{session_id}.json"` (built by `get_context_file_path()` at line 143).

---

### What Phases 2-5 Need to Connect

**Phase 2 (T1): `operations.py` intermediary layer.** `server.py` never calls `github.py` directly — all calls route through `operations.py`. This pattern is established in all existing tools: `server.py` imports `from . import operations` and calls `operations.add_item()`, etc. T1 adds four new functions to `operations.py`:

- `create_sam_task(parent_issue_number, task_id, feature, task_type, agent, priority, skills, dependencies, description, acceptance_criteria, labels, output)` → `dict`. Calls `get_github()` (raises `GitHubUnavailableError` on failure), constructs `SamTask`, calls `github.create_task_issue()`. Returns `{"issue_number": N, "title": "...", "url": "..."}`.
- `get_sam_tasks(parent_issue_number, refresh_cache=True, output)` → `dict`. Calls `try_get_github()` (returns `None` offline). For each `SubIssue` from `get_task_issues()`, fetches body via `repo.get_issue(si.sub_issue.number)` (NOT directly on `SubIssue`), calls `parse_sam_task_metadata(body)`. If `refresh_cache=True` and tasks non-empty, writes `.claude/context/sam-tasks-{feature_slug}.json`. Returns `{"tasks": [...], "count": N, "parent_issue_number": N}`.
- `update_sam_task_status(issue_number, new_status, output)` → `dict`. Calls `get_github()`, calls `github.update_task_status()`. Returns `{"updated": bool, "issue_number": N, "new_status": "..."}`.
- `get_ready_sam_tasks(parent_issue_number, output)` → `dict`. Calls `get_sam_tasks()`, converts task dicts to a minimal structure compatible with readiness logic, applies the same three-line dependency resolution as `get_ready_tasks()` (do NOT import `implementation_manager.py`; re-implement inline). Returns `{"feature": "slug", "ready_tasks": [...], "count": N}` where each ready task includes `issue_number`.

The `out = output or Output()` pattern is used in every function. `GithubException` is already caught inside `github.py` functions — do NOT double-catch in `operations.py`. Only `BacklogError` subclasses propagate upward to the server boundary.

**Phase 2 (T2): MCP tools in `server.py`.** Four new `@mcp.tool()` async functions following the `backlog_add` exemplar (lines 17-53 of `server.py`). Invariants: `async def`, `Annotated[type, Field(description="...")]` parameters, `Output()` instance, `await asyncio.to_thread(operations.func, ...)`, return `{**result, **out.to_dict()}`, catch `BacklogError` and return `{"error": str(e), **out.to_dict()}`. Tool names: `backlog_create_sam_task`, `backlog_get_sam_tasks`, `backlog_update_sam_task_status`, `backlog_get_ready_sam_tasks`.

**Phase 3 (T3): `--github` flag in `implementation_manager.py`.** The `ready-tasks` and `status` commands gain optional `--github` (bool) and `--parent-issue` (int) parameters. When `--github` is set, a new `fetch_tasks_from_github(parent_issue_number, feature_slug, cache_path) -> list[Task] | None` function is called before `get_ready_tasks()`. This function imports `backlog_core.github` via `sys.path.insert`. The path from `implementation_manager.py` to `backlog_core` is 6 directories up: `Path(__file__).resolve().parents[6] / ".claude" / "skills" / "backlog"` — verify this against the actual directory before writing. The import is guarded: `if _BACKLOG_CORE.exists(): sys.path.insert(0, str(_BACKLOG_CORE.parent))`. Offline fallback: when GitHub is unavailable, reads from `.claude/context/sam-tasks-{slug}.json` cache. When both are absent, returns `None`; caller falls back to local YAML files. The `get_ready_tasks()` function body is unchanged. The `Task` and `TaskStatus` types are unchanged.

**Phase 4 (T4): GitHub sync in `task_status_hook.py`.** Two new functions added:

- `get_parent_issue_number(hook_input) -> int | None` — reads `parent_issue_number` key from the active-task context file. Never raises. Returns `None` when field absent or file missing.
- `sync_completion_to_github(task_file_path, task_id, parent_issue_number) -> None` — reads `github_issue` field from the task YAML frontmatter. If absent, logs warning and returns. Otherwise: adds `backlog_core` parent dir to `sys.path` (same guard pattern), calls `get_github()`, calls `update_task_status(repo, github_issue_number, "complete")`. Wraps everything in `try/except Exception` — failure logs to stderr and returns. This function is always called AFTER the local YAML write succeeds; GitHub failure never changes exit code.

The existing `handle_subagent_stop` call order is: parse prompt → fallback to context file → resolve task file → write local YAML → delete context file → `sync_completion_to_github()` (new, step 6). The `handle_activity_update` (PostToolUse) path is NOT changed — no GitHub call on keystrokes.

**Phase 5 (T5): Skill doc updates.** Both `.claude/skills/implement-feature/SKILL.md` and `plugins/python3-development/skills/development/implement-feature/SKILL.md` must be updated identically. Same for the two copies of `start-task/SKILL.md`. There is no symlink or include mechanism — manual sync required.

---

### Key Patterns to Follow

**`operations.py` function skeleton:**
...
def create_sam_task(
    ...
    repo = get_github()  # raises GitHubUnavailableError for write ops
    # ... call github.py function ...
    return {"key": value, **out.to_dict()}
```

**`server.py` tool skeleton:**

```python
@mcp.tool()
async def backlog_create_sam_task(
    parent_issue_number: Annotated[int, Field(description="Parent story issue number (without #)")],
    # ... all params with Annotated[type, Field(description="...")] ...
) -> dict:
    """Docstring: when to use and what it returns."""
    out = Output()
    try:
...
**`implementation_manager.py` `sys.path` insert pattern (existing, at line 36):**

```python
sys.path.insert(0, str(Path(__file__).resolve().parent))
```

The new `backlog_core` insert follows the same pattern but with the resolved path to the backlog package's parent directory. Verify the actual depth by counting: `scripts/` → `implementation-manager/` → `skills/` → `python3-development/` → `plugins/` → project root → `.claude/` → `skills/` → `backlog/`. The `backlog_core` directory is a sibling of `backlog_core`'s parent `backlog/` — so `sys.path.insert(0, str(_BACKLOG_CORE.parent))` makes `import backlog_core.github` work.

**`task_status_hook.py` `task_format` import (existing, at line 34):**

```python
from task_format import has_yaml_frontmatter, normalize_status, parse_yaml_frontmatter, update_yaml_field
```

No `sys.path.insert` for `task_format` because `task_status_hook.py` is called by Claude Code with `python3 "./plugins/python3-development/skills/implementation-manager/scripts/task_status_hook.py"` from the project root — and Claude Code adds the script's directory to `sys.path` automatically when using the `command:` hook type.

**YAML library constraint.** All YAML reads and writes use `ruamel.yaml`. The `import yaml` (pyyaml) is forbidden. Cache files (`.claude/context/sam-tasks-{slug}.json`) use `json` stdlib, not YAML.

**Error handling exit codes.** Hook exit 0 = success or best-effort (GitHub failure, cache miss). Hook exit 2 = hard error surfaced to agent (task file not found, YAML parse failure). GitHub sync failure is always exit 0.

**New YAML field in task frontmatter.** A new optional field `github_issue: N` (integer) is written to task YAML frontmatter after `create_task_issue()` succeeds (by the migration script T6) or by T4's hook. This field is read by `task_status_hook.py` `SubagentStop` handler to find the issue number for `update_task_status()`. Absent field = skip GitHub sync with a warning.

**New field in active-task context file.** `.claude/context/active-task-{sid}.json` gains an optional `parent_issue_number` field:
...
`/start-task` SKILL.md step 5 must be updated to write this field. The hook reads it via `get_parent_issue_number(hook_input)`.

**GitHub sub-issue cache file.** Location: `.claude/context/sam-tasks-{feature_slug}.json`. Written by `get_sam_tasks()` in `operations.py` and `fetch_tasks_from_github()` in `implementation_manager.py` after successful GitHub fetches. Read by both when `try_get_github()` returns `None` (offline). Format:
...
- **PyGitHub sub-issue API.** Verified in installed PyGitHub source at `.venv/lib/python3.11/site-packages/github/Issue.py` (copyright 2025). Methods: `issue.get_sub_issues()`, `issue.add_sub_issue(issue_obj)`, `issue.prioritize_sub_issue(sub_issue, after_sub_issue)`. Sub-issue plan availability on the repository is accepted risk (Option A body cross-refs is the stated fallback).
- **`backlog_core` import path.** `operations.py`, `github.py`, `models.py`, `parsing.py`, `server.py` are all siblings in `.claude/skills/backlog/backlog_core/`. When `implementation_manager.py` and `task_status_hook.py` import `backlog_core`, they must add the `backlog/` directory (parent of `backlog_core/`) to `sys.path` — NOT the `backlog_core/` directory itself.
- **FastMCP 3.x.** `server.py` uses `from fastmcp import Context, FastMCP`. New tools follow the same import pattern.
- **`ruamel.yaml`.** Used in all YAML operations in `backlog_core`. Forbidden: `import yaml` (pyyaml).

---

### Frozen Files — Must Not Be Modified

These files must not be touched during Phases 2-5:

| File | Why frozen |
|---|---|
| `github.py` lines 455-581 (`create_task_issue`, `get_task_issues`, `update_task_status`) | Phase 1 complete (commit 30066de) |
| `models.py` (`SamTask`, `Output`, `BacklogError`, `GitHubUnavailableError`) | Phase 1 complete |
| `parsing.py` (`parse_sam_task_metadata`, `build_sam_task_body`, `build_sam_task_issue_title`) | Phase 1 complete |
| `task_format.py` | Shared module; changes affect both `implementation_manager.py` and `task_status_hook.py` |
| `get_ready_tasks()` in `implementation_manager.py` | Algorithm correct; only input source changes, not function body |
| `handle_activity_update()` in `task_status_hook.py` | No GitHub call on PostToolUse — hard latency constraint |
| `claim-task` command in `implementation_manager.py` | Atomic local write; no GitHub side-effect added |
